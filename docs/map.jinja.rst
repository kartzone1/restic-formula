...
We will see a quick setup to configure the ``restic`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``restic`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``restic`` formulas under ``/srv/salt/restic/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/restic/parameters/dns:domain/
    ln -s dns:domain /srv/salt/restic/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/restic/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/restic/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``restic`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``restic`` formulas under ``/srv/salt/restic/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/restic/parameters/roles
...
We create a configuration for the role ``restic/server`` in ``/srv/salt/restic/parameters/roles/restic/server.yaml``:
...
We create another configuration for the role ``restic/client`` in ``/srv/salt/restic/parameters/roles/restic/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles restic/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles restic/client
...
.. code-block:: console
    ...
    mv /srv/salt/restic/parameters/dns:domain/  '/srv/salt/restic/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-restic-as-json:
      test.nop:
        - name: {{ restic | json }}
...
Here is an example based on `template-formula/restic/config/file.sls`_:
...
.. code-block:: sls
    ...
    restic-config-file-file-managed:
      file.managed:
        - name: {{ restic.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='restic-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``restic`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ restic['winner'] }}
...
.. _template-formula/restic/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/restic/config/file.sls
